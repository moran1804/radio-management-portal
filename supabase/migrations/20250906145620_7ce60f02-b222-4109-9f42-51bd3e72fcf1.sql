-- Update the generate_recurring_shows function to handle UK timezone properly
...
DECLARE
  ...
  uk_timezone TEXT := 'Europe/London';
BEGIN
  ...
  LOOP
    ...
    WHILE weeks_generated < p_weeks_ahead LOOP
      ...
      IF target_date >= CURRENT_DATE THEN
        -- Calculate show start and end times in UK timezone first
        -- Combine the target date with the slot's start time
        show_start := (target_date || ' ' || slot_record.start_time)::TIMESTAMP;
        
        -- Convert from UK time to UTC for storage
        show_start := show_start AT TIME ZONE uk_timezone AT TIME ZONE 'UTC';
