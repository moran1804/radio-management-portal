-- Fix the generate_recurring_shows function to properly handle UK timezone conversion
  ...
  LOOP
    ...
    WHILE weeks_generated < p_weeks_ahead LOOP
      ...
      IF target_date >= CURRENT_DATE THEN
        -- Create a local timestamp by combining date and time
        local_timestamp := (target_date || ' ' || slot_record.start_time)::TIMESTAMP;
        
        -- Convert from UK local time to UTC by treating it as a UK timestamp
        show_start := local_timestamp AT TIME ZONE uk_timezone;
